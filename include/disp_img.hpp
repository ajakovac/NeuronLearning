...
#ifndef INCLUDE_DISP_IMG_HPP_
#define INCLUDE_DISP_IMG_HPP_
...
Image_View::Image_View(int hin, int win)
  :  _width(win), _height(hin), opened(false), window(), sprite(), _title("") {
  f = [](Image_View&) -> void {};
  img.create(win, hin);
}
...
void Image_View::show() {
  ...
  if (_title == "")
    ...
    window.create(sf::VideoMode(_width, _height), _title.c_str());
  f(*this);
  update_image();
  while (window.isOpen()) {
    sf::Event event;
    while (window.pollEvent(event)) {
      ...
      if (event.key.code == sf::Keyboard::Escape) close();
        ...
        if (event.key.code == sf::Keyboard::Return) f(*this);
      }
    }
    window.clear();
    window.draw(sprite);
