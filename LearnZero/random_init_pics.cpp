...
int main(int argc, char const *argv[]) try {
  ...
  int Nrnn = 10000;
  std::cout << "Save result of " << Nrnn << " random image" << std::endl;
  std::ofstream frnn;
  frnn.open("tr.dat");
  if (!frnn.is_open()) throw(Error("File can not be opened!"));
  for (int rnn = 0; rnn < Nrnn; ++rnn) {
    // setting initial conditions: uniform random numbers in (0,1)
    for (int n = 0; n < nrows*ncols; ++n)
      ntw.site(basely, n) = uniform_dist(0, 1.0)();
  ...
  for (int i = 0; i< 10; i++) frnn.close();

  // mnist.next();
  // for (int n = 0; n < nrows*ncols; ++n)
  //   ntw.site(basely, n) = data[n]/255.0;
  int* imdat = new int[nrows*ncols];
  auto rL = ntw.readLayer(basely);
  for (uint n = 0; n < ntw.nSitesinLayer(basely); ++n)
    imdat[n] = 255*rL(n);

  auto update3 = [=](Image_View& v) {
    v.fillGreyscale(imdat);
    v.setTitle("Initial image");
  };
  Image_View MNIST_view(nrows, ncols, update3);
  MNIST_view.show();
  ...
  std::cout << "Steepest descent evolution of the last image to (0,0) result";
  std::cout << std::endl;
  int Niter = 100000;
  double learningrate = 10.0;  // average learning rate
  for (int nit = 0; nit < Niter; ++nit) {
    ...
    bpntw.reset();
    backpropagate(&bpntw, 0.0);
    // bpntwadd.add(bpntw, 1.0);  // now we use direct steepest descent
    ntw.forallSitesinLayer(basely, [&](int n){
      ntw[n] -= learningrate*bpntw.Dsite(n)*ntw[n]*(1-ntw[n])*(1-ntw[n]);
    });
  }
  for (uint n = 0; n < ntw.nSitesinLayer(basely); ++n)
    imdat[n] = 255*rL(n);
  ...
  // int epochnumber = 15;  // train in epochs
